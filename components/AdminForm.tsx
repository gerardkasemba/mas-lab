...
// Helper function to generate UUID (more reliable than crypto.randomUUID())
const generateUUID = () => {
...
const validateVideoDuration = (file: File): Promise<boolean> => {
  return new Promise((resolve) => {
    ...
    video.onloadedmetadata = function() {
      ...
      resolve(video.duration <= 600); // 10 minutes = 600 seconds
    };
    
    video.onerror = function() {
      // If we can't determine duration, allow the upload but warn the user
    ...
    try {
      ...
      for (const file of formData.media) {
        if (file.type.includes('video')) {
          ...
          if (!isValid) {
            setErrors({ media: 'Videos must be less than 10 minutes long' });
            setUploading(false);
            return;
          }
        }
      ...
      let avatar_url = "";
      if (formData.avatar) {
        const avatarPath = `avatars/${generateUUID()}/${formData.avatar.name}`;
        const { data, error } = await supabase.storage
          .from("lab-upload")
          .upload(avatarPath, formData.avatar);
        if (error) {
          console.error("Avatar upload error:", error);
          setErrors({ avatar: 'Failed to upload avatar image' });
          setUploading(false);
          return;
        }
        if (data) {
      ...
      if (artistError) {
        ...
        setErrors({ general: 'Failed to save artist information' });
        setUploading(false);
        return;
      }
      ...
      if (artist) {
        ...
        if (frameworkError) {
          ...
          setErrors({ general: 'Failed to save framework data' });
          setUploading(false);
          return;
        }
        ...
        if (sessionError) {
          ...
          setErrors({ general: 'Failed to save session data' });
          setUploading(false);
          return;
        }
        ...
        for (const file of formData.media) {
          const mediaPath = `media/${artist.id}/${generateUUID()}-${file.name}`;
          const { data, error } = await supabase.storage
            .from("lab-upload")
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input
          type="text"
          value={formData.goals}  // Fixed: was using formData.values
      ...
      <div>
        <label className="block">Media Files (any type, videos must be under 10 minutes)</label>
        <input
          type="file"
          multiple
          accept="*/*"
      ...
      <button 
        ...
        disabled={uploading}
      >
        {uploading ? 'Uploading...' : 'Save Artist'}
      </button>
    </form>
  );
}